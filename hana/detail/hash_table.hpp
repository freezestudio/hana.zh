...
BOOST_HANA_NAMESPACE_BEGIN namespace detail {
    ...
    struct find_indices {
		// vs2017 added
		using tHash_t = decltype(hana::hash(std::declval<Key>()));
		using Hash = typename tHash_t::type;
	...
	template <template <std::size_t> class KeyAtIndex, std::size_t N, std::size_t ...i>
	struct make_hash_table<KeyAtIndex, N, std::index_sequence<i...>> {
		using type_temp = decltype(hana::hash(std::declval<KeyAtIndex<i>>()));
		using type = hash_table<
			bucket<typename type_temp::type, i>...
		>;
	};
