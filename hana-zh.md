...
这样做有几个原因。首先，哈纳是一个非常基础的图书馆;我们基本上重新实现了核心语言和支持异构类型的标准库。当经历代码时，人们很快意识到，其他库很少需要，几乎一切都必须从头开始实现。此外，由于`Hana`是非常基本的，因此更有动机保持依赖最小，因为那些依赖将传递给用户。关于对`Boost`最小的依赖，使用它的一个大论据是可移植性。然而，作为一个前沿库，Hana只定位最近的编译器。因此，我们可以依赖于现代构造，并且通过使用Boost提供给我们的可移植性将主要代表自重。
